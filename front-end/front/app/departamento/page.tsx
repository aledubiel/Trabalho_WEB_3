...
import { getDepartamentos, buscarDepartamentosPorNome, Departamento } from "@/lib/departamentoApi";
...
export default function DepartamentosPage() {
  ...
  const fetchAllDepartamentos = async () => {
    ...
    try {
      ...
      setError("Não foi possível carregar os departamentos. Verifique se a API está rodando e o CORS está habilitado.");
      console.error(err);
      setDepartamentos([]);
    } finally {
      setLoading(false);
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      const response = await buscarDepartamentosPorNome(searchTerm);
      setDepartamentos(response.data);
      setIsSearchResult(true);
    } catch (err: any) {
      if (err.response && err.response.status === 404) {
        setError(`Nenhum departamento encontrado com o nome "${searchTerm}".`);
      } else {
  ...
  const renderContent = () => {
    ...
    if (departamentos.length === 0) {
      return (
        <div className="text-center py-20">
          <p className="text-lg text-gray-500">Nenhum departamento para exibir.</p>
        </div>
      )
    }
          ...
          >
            <div>
              ...
              <div className="space-y-3 text-gray-600">
                ...
                {/* Adicione mais campos do departamento se necessário */}
              </div>
            </div>
            <div className="mt-6">
              <Link href={`/departamento/${departamento.id}`}>
      ...
      <div className="mb-8 p-4 bg-white rounded-lg shadow-sm border">
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4 items-center">
          <input
            ...
            placeholder="Procurar por nome do departamento..."
